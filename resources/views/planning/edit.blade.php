                ...
                <form method="POST" action="{{ route('planning.update', $plan->id) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="component_name" class="form-control"
                            value="{{ $plan->component_name }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select" required>
                            ...
                            <option value="not-working" {{ $plan->status == 'not-working' ? 'selected' : '' }}>Not Working
                            </option>
                            <option value="pending" {{ $plan->status == 'pending' ? 'selected' : '' }}>Pending Purchase
                            </option>
                            <option value="purchased" {{ $plan->status == 'purchased' ? 'selected' : '' }}>Purchased
                            </option>
                        </select>
                    </div>

                    <x-primary-button type="submit">Update Plan</x-primary-button>
                    <a href="{{ route('planning.index') }}" class="btn btn-secondary ms-2"
                        hx-get="{{ route('planning.index') }}" hx-push-url="{{ route('planning.index') }}"
